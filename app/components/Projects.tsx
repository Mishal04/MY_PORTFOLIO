...
const projects = [
  ...
  },
  {
    ...
    year: "2024",
    description:
      "A premium real estate platform to discover and explore curated properties — from cosy apartments to grand estates. Features property search, city/neighbourhood filtering, and a clean modern UI.",
...
export default function Projects() {
  ...
  // Set-based lookup — O(1) instead of Array.includes inside render loop
  const visibleSet = new Set(FILTER_INDICES[activeFilter]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6 w-full">
            <AnimatePresence>
              {projects.slice(1).map((project, i) => {
                const realIndex = i + 1;
                if (!visibleSet.has(realIndex)) return null;
                return (
                  <m.a
                    key={project.title}
                    href={project.link}
